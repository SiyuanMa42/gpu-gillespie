...
def gillespie_kernel(
    ...
    while current_time < max_time and time_idx < n_timepoints:
        ...
        while (time_idx < n_timepoints and 
...
def launch_gillespie_simulation(
    ...
    d_species = cuda.to_device(
        np.tile(initial_species, (n_trajectories, 1)).astype(np.int32)
    )
    ...
    if use_tau_leaping:
        tau_leaping_kernel[blocks_per_grid, threads_per_block](
            d_species, d_reactions, d_stoichiometry, d_rate_constants,
            d_time_points, d_results, max_time, n_trajectories,
            n_species, n_reactions, n_timepoints
        )
    else:
